  ...
  rules: {
    ...
    _item: ($) =>
      choice(
        ...
        seq(token(prec(1, "subrules")), $._whitespace, $.subrules),
    ...
    dash_line: (_) => seq(token(prec(1, "----")), token.immediate(repeat("-"))),

    rule_line: ($) => seq(repeat1($._element), $._line_end),
    ...
    _string_desc_list1: ($) =>
      seq($.string_desc, repeat(seq(",", $.string_desc))),
    string_desc: ($) =>
      seq(alias(/[a-zA-Z\d_]+/, $.string), repeat($.homomorphism)),
    ...
    _hom_inner: ($) =>
      choice(
        seq(
          ...
          repeat($._hom_inner_string_left),
          optional(seq($.dots, repeat1($._hom_inner_string_right))),
        ),
        seq(
          "</",
          repeat($._hom_inner_comp_left),
          "//",
          $.comprehension_bound,
          "/>",
        ),
      ...
      seq(
        ...
        optional(
          seq(
            "IN",
            $._comprehension_string,
            optional(seq($.dots, $._comprehension_string)),
          ),
        ),
      ),
